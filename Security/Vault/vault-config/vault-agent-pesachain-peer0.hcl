auto_auth {
  method {
    type = "approle"
    config = {
      role_id_file_path = "/Vault/vault-creds/pesachain-peer0-role_roleid.txt"
      secret_id_file_path = "/Vault/vault-creds/pesachain-peer0-role_secretid.txt"
...
template {
  source      = "/Vault/vault-tmpl/pesachain/peer0-prvKey.ctmpl"
  destination = "/Vault/vault-secrets/peer/msp/keystore/prvKey.pem"
  token_file  = "/Vault/vault-tokens/peer/token.txt"
}

template {
  source      = "/Vault/vault-tmpl/pesachain/peer0-signcert.ctmpl"
  destination = "/Vault/vault-secrets/peer/msp/signcerts/cert.pem"
  token_file  = "/Vault/vault-tokens/peer/token.txt"
}

template {
  source      = "/Vault/vault-tmpl/pesachain/peer0-srvCert.ctmpl"
  destination = "/Vault/vault-secrets/peer/tls/server.crt"
  token_file  = "/Vault/vault-tokens/peer/token.txt"
}

template {
  source      = "/Vault/vault-tmpl/pesachain/peer0-srvKey.ctmpl"
  destination = "/Vault/vault-secrets/peer/tls/server.key"
  token_file  = "/Vault/vault-tokens/peer/token.txt"
}

template {
  source      = "/Vault/vault-tmpl/pesachain/peer0-caCert.ctmpl"
